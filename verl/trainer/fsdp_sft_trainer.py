...
def convert_to_regular_types(obj):
    ...
    if isinstance(obj, (ListConfig, DictConfig)):
        return {
            k: convert_to_regular_types(v)
            for k, v in obj.items()
        } if isinstance(obj, DictConfig) else list(obj)
...
class FSDPSFTTrainer(object):
    ...
    def __init__(self, config, device_mesh: DeviceMesh,
        ...
        self.use_remove_padding = getattr(self.config, 'use_remove_padding',
                                          False)
    ...
    def _normalize_config_bsz(self):
        dp_size = self.device_mesh.size(
            0
        ) if not self.ulysses_device_mesh else self.ulysses_device_mesh.size(0)
        if self.device_mesh.get_rank() == 0:
    ...
    def _build_model_optimizer(self):
        ...
        num_warmup_steps = int(self.total_steps *
            ...
            with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                if not use_sp:
                    ...
                    input_ids_rmpad_rolled = torch.roll(
                        input_ids_rmpad, shifts=-1, dims=1)  # (1, total_nnz)
                    ...
                    output = self.fsdp_model(
                        ...
                        attention_mask=
                        None,  # Not needed with flash attention varlen
                    ...
                    loss = gather_outpus_and_unpad(loss,
                                                   gather_dim=0,
                                                   unpad_dim=0,
                    ...
                    full_loss = pad_input(hidden_states=loss.unsqueeze(-1),
                                          ...
                                          batch=batch_size,
                                          seqlen=seqlen)
                    full_loss = full_loss.squeeze(
                        -1)[:, :-1]  # Remove last token's loss
                ...
                if self.config.data.balance_dp_token:
                    ...
                    dp_size = self.ulysses_device_mesh.size(
                        'dp') if use_sp else torch.distributed.get_world_size(
                        )
                else:
                    dp_size = 1
    ...
    def save_checkpoint(self, step):
        ...
        with FSDP.state_dict_type(self.fsdp_model,
                                  StateDictType.FULL_STATE_DICT, cfg):
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for data in tqdm(
                ...
                data = TensorDict(
                    data, batch_size=self.config.data.train_batch_size).cuda()
                ...
                if global_step >= self.total_training_steps:
                    ...
                    for val_data in self.val_dataloader:
                        val_data = TensorDict(val_data,
                                              batch_size=self.config.data.
                                              micro_batch_size_per_gpu).cuda()
