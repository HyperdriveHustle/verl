...
from verl.experimental.reward.remote_reward_model import (
...
@ray.remote
class RemoteRewardManagerWorker:
    ...
    def _init_reward_fn(self):
        ...
        logger.info(f"RemoteRewardManagerWorker initialized with remote LLM-judge: {BASE_URL}, model: {MODEL_NAME}, timeout: {OPENAI_TIMEOUT}s, max_retries: {OPENAI_MAX_RETRIES}")
    ...
    async def compute_score(self, data: DataProto) -> dict:
        ...
        if problem_text is None and "raw_prompt" in data_item.non_tensor_batch:
            raw_prompt = data_item.non_tensor_batch["raw_prompt"].tolist()
            problem_text = raw_prompt[-1].get("content", None)
        ...
        if problem_text is None:
            ...
            if candidate and candidate != 'None':
                ...
                # 最后的备选：从 prompt_key 中提取
                prompt = data_item.non_tensor_batch.get(self.config.data.prompt_key, None)
                if prompt:
        ...
        result = {
            ...
            "rm_score": float(reward),
            "reward_extra_info": reward_extra_info,
        }
        
        # print(
        #     f"🎁 [RemoteRewardManagerWorker.compute_score] "
