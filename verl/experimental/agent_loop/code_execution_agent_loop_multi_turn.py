...
class CodeExecutionAgentLoop_Multi_turn(AgentLoopBase):
    ...
    async def run(self, sampling_params, **kwargs) -> AgentLoopOutput:
        # logger.info("[taro_debug]**************agent run start**************")
        messages = kwargs['raw_prompt']
        
        metrics = {}
        extra_fields = {"extracted_code": None, "tool_call_response": None}
        ...
        while turns < cur_max_turns:
            ...
            extra_fields['response_length'] = len(response_ids)
            if output.log_probs:
            ...
            if extracted_code and hasattr(self, 'code_tool'):
                ...
                with simple_timer(f"tool_calls", metrics):
                    instance_id = None
                    try:
                        instance_id, _ = await self.code_tool.create()
                        ...
                        extra_fields['tool_call_response'] = json.dumps(meta_data, ensure_ascii=False)
                        tool_pass_fail_lists.append(meta_data.get("pass_fail_list", [0] * len(ground_truth.get("inputs",[]))))
                        if meta_data["status"] == "timeout":
                            metrics["timeout"] = 1
                            error_message = yaml.safe_load(open("recipe/async_dapo_tool/deps/multi_turn_feedback.yaml"))['timeout'].format(extracted_code=extracted_code)
                        else:
                            ...
                            if score.lower() == "success" and pass_rate == 1.0:
                                ...
                                error_message = yaml.safe_load(open("recipe/async_dapo_tool/deps/multi_turn_feedback.yaml"))['error'].format(stdout=stdout, stderr=stderr)
                    except Exception as e:
                        logger.error(f"Error during reward calculation: {e}")
                        extra_fields['tool_call_response'] = json.dumps({"success": False, "error": repr(e)})
                ...
                extra_fields['tool_call_response'] = None
                answer_reward = 0
