...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(self, micro_batch,
                             temperature) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Returns: 
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                input_ids_rmpad_rolled = torch.roll(input_ids_rmpad,
                                                    shifts=-1,
                                                    dims=1)  # (1, total_nnz)
                ...
                if self.use_ulysses_sp:
                    ...
                    log_probs = gather_outpus_and_unpad(log_probs,
                                                        gather_dim=0,
                                                        unpad_dim=0,
                ...
                entropy = full_entropy.squeeze(
                    -1)[:, -response_length - 1:-1]  # (bsz, response_length)
                log_probs = full_log_probs.squeeze(
                    -1)[:, -response_length - 1:-1]  # (bsz, response_length)
    ...
    def compute_log_prob(self, data: DataProto) -> torch.Tensor:
        ...
        temperature = data.meta_info[
            'temperature']  # temperature must be in the data.meta_info to avoid slient error
        ...
        if has_multi_modal_inputs:
            ...
            max_token_len = data.meta_info[
                'max_token_len'] * self.ulysses_sequence_parallel_size
        ...
        if use_dynamic_bsz:
            ...
            assert len(indices) == log_probs.size(
                0), f"{len(indices)} vs. {log_probs.size()}"
    ...
    def update_policy(self, data: DataProto):
        ...
        if has_multi_modal_inputs:
            num_mini_batches = data.batch.batch_size[
                0] // self.config.ppo_mini_batch_size
            non_tensor_select_keys = ['multi_modal_inputs']
        ...
        print(
            f'update actor: {self.config.ppo_mini_batch_size=} {self.config.ppo_epochs=} {has_multi_modal_inputs=} {self.config.use_dynamic_bsz=} {self.config.ppo_micro_batch_size_per_gpu=} {batch.batch_size=}'
        )
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                if has_multi_modal_inputs:
                    ...
                    num_micro_batches = mini_batch.batch.batch_size[
                        0] // self.config.ppo_micro_batch_size_per_gpu
                    micro_batches = data.select(
                        select_keys,
                        non_tensor_select_keys).chunk(num_micro_batches)
