...
class AllGatherPPModel:
    ...
    def __init__(self, model_provider) -> None:
        ...
        rank_list[self.pp_rank], rank_list[-1] = rank_list[-1], rank_list[
            self.pp_rank]
        ...
        for cur_pp_rank in rank_list:
            ...
            if cur_pp_rank != self.pp_rank:
                ...
                assert len(
                    models
                ) == self._model_chunk_size, f"{len(models)} != {self._model_chunk_size}"
    ...
    def _build_param_buffer(self, pp_rank):
        ...
        if pp_rank == self._pp_rank:
            ...
            self.memory_buffers[pp_rank] = {
                torch.bfloat16:
                MemoryBuffer(source.numel(), source.numel(), torch.bfloat16,
                             source)
            }
        else:
    ...
    def _build_param_references(self, pp_rank, maintain_weight=False):
        if pp_rank == self._pp_rank:
            return
        model = self.pp_models[pp_rank]
        build_memory_reference_from_module(model,
                                           self.memory_buffers[pp_rank],
    ...
    def allgather_params(self):
        ...
        for cur_pp_rank in range(self.pp_size):
            global_src = dist.get_global_rank(group=self.pp_group,
                                              group_rank=cur_pp_rank)

            # NOTE(sgm): the async op may cause memory leakage of the memory_buffer/pp_models

            for _, param in sorted(
                    self.pp_models[cur_pp_rank].named_parameters()):
    ...
    def forward(self, *inputs, **kwargs):
        try:
            ...
            for cur_chunk_rank in range(self._model_chunk_size):
                if self._vpp_size:
                    mpu.set_virtual_pipeline_model_parallel_rank(
                        cur_chunk_rank)

                for cur_pp_rank in range(self.pp_size):
                    mpu.set_pipeline_model_parallel_rank(cur_pp_rank)
                    self.pp_models[cur_pp_rank][
                        cur_chunk_rank].set_input_tensor(prev_output)
                    ret = self.pp_models[cur_pp_rank][cur_chunk_rank](*inputs,
                                                                      **kwargs)
                    self.pp_models[cur_pp_rank][
                        cur_chunk_rank].set_input_tensor(None)
                    prev_output = ret
        finally:
            if self._vpp_size:
        ...
        for pp_rank in range(self.pp_size):
            ...
            for model_chunk_idx in range(len(self.pp_models[pp_rank])):
                ...
                pp_model = unwrap_model(
                    pp_model, ((torchDDP, LocalDDP,
                                Float16Module)))  # not use Float16Module
    ...
    def update_this_rank_models(self, new_models):
        ...
        self._pp_models[self.pp_rank] = unwrap_model(new_models,
                                                     (torchDDP, LocalDDP))
...
class MegatronVLLMShardingManager(BaseShardingManager):
    ...
    def __init__(self, module: AllGatherPPModel, inference_engine: LLM,
                 model_config, layer_name_mapping):
        self.module = module
    ...
    def default_tp_concat_fn(self, name, param, infer_params, model_config):
        ...
        if self.layer_name_mapping.get("qkv_layer_name") in name:
            ...
            split_size = [
                kv_size_per_tp * num_q_per_kv, kv_size_per_tp, kv_size_per_tp
            ]
    ...
    def _post_process_params(self, params):
        ...
        for name in params.keys():
            ...
            if tp_utils.is_tensor_parallel_param(param):
                ...
                infer_params = [
                    torch.empty_like(param) for _ in range(micro_dp_size)
                ]
                torch.distributed.all_gather(infer_params,
                                             param,
                                             group=micro_dp_group)
    ...
    def __enter__(self):
        ...
        self.inference_engine.sync_model_weights(self.params,
