...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        if role == 'actor':
            ...
            num_warmup_steps_ratio = optim_config.get('lr_warmup_steps_ratio',
                                                      0.)
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        init_context = get_init_weight_context_manager()
        print(
            f"prepare for pretrained | 进程 PID={os.getpid()} | 设备={torch.cuda.current_device()} | Rank={self.rank}"
        )
        with init_context(), warnings.catch_warnings():
            warnings.simplefilter("ignore")
            ...
            critic_module = AutoModelForTokenClassification.from_pretrained(
                ...
                trust_remote_code=trust_remote_code)
            print(
                f"finish from pretrained | 进程 PID={os.getpid()} | 设备={torch.cuda.current_device()} | Rank={self.rank}"
            )
        ...
        sharding_strategy = get_sharding_strategy(fsdp_mesh)
        print(
            f"prepare for FSDP | 进程 PID={os.getpid()} | 设备={torch.cuda.current_device()} | Rank={self.rank}"
        )
