        ...
        if "tele" in local_path.lower():
            for name, param in self.model.named_parameters():
                if any(nd in name for nd in ["self_attn.q_proj.bias", "self_attn.k_proj.bias", 
                    ...
                    print(f"> telechat-rank-{self.rank}: Freezing parameter {name}")
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        with warnings.catch_warnings():
            ...
            with FSDP.state_dict_type(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
                ...
                # @xiaohuihu: Print specific tensor values to observe if they're being updated during training
                if "tele" in local_path.lower():
                    print(f"[rank-{self.rank}]: Checking frozen parameters in saved checkpoint:")
                    frozen_param_names = ["self_attn.q_proj.bias", "self_attn.k_proj.bias", "self_attn.v_proj.bias", "mlp.gate_proj.bias", "mlp.up_proj.bias"]
                    for name, param in self.model.named_parameters():
                        for frozen_name in frozen_param_names:
                            if frozen_name in name:
                                print(f"  > telechat-rank-{self.rank}: {name} - requires_grad={param.requires_grad}")
                                # Print some statistics about the parameter
                                if param.numel() > 0:
