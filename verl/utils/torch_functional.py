...
def logprobs_from_logits_v2(logits: torch.FloatTensor, labels):
    ...
    if logits.dtype in [torch.float32, torch.float64]:
        logits_labels = torch.gather(logits,
                                     dim=-1,
        ...
        for row_logits, row_labels in zip(
                logits, labels):  # loop to reduce peak mem consumption
...
def get_eos_mask(response_id: torch.Tensor,
...
def compute_grad_norm(model: nn.Module):
    ...
    for param in model.parameters():
        if param.grad is not None:
            total_grad_square += torch.sum(torch.square(
                param.grad.detach())).item()
    return total_grad_square


def broadcast_dict_tensor(tensors: Union[Dict[str, torch.Tensor], TensorDict],
                          src, group):
    """
    ...
    for key in tensors.sorted_keys:
        torch.distributed.broadcast(tensors[key],
                                    src=src,
                                    group=group,
                                    async_op=False)


def allgather_dict_tensors(tensors: Union[Dict[str, torch.Tensor], TensorDict],
                           size,
                           group,
                           dim=0):
    """
    ...
    for key in sorted_keys:
        ...
        torch.distributed.all_gather(output[key],
                                     val,
                                     group=group,
                                     async_op=False)
...
def split_dict_tensor_into_batches(tensors: TensorDict,
...
def pad_sequence_to_length(tensors, max_seq_len, pad_token_id, left_pad=False):
    ...
    if tensors.shape[-1] >= max_seq_len:
        return tensors
    pad_tuple = (max_seq_len - tensors.shape[-1],
                 0) if left_pad else (0, max_seq_len - tensors.shape[-1])
...
def log_probs_from_logits_response_rmpad(input_ids, attention_mask,
                                         logits_rmpad, response_length):
    ...
    full_output = pad_input(hidden_states=full_log_probs_rmpad.unsqueeze(-1),
                            ...
                            batch=batch_size,
                            seqlen=seqlen)
    output = full_output.squeeze(-1)[:, -response_length -
                                     1:-1]  # [batch_size, response_length]
    return output


def log_probs_from_logits_all_rmpad(input_ids_rmpad, logits_rmpad, indices,
                                    batch_size, seqlen, response_length):
