...
def query_ray_cluster():
    ...
    if not ray.is_initialized():
        ...
        print(
            f"Connected to Ray cluster at {ray.get_runtime_context().gcs_address}"
        )
...
class MLPLayerWorker(MegatronWorker):
    ...
    def __init__(self):
        ...
        print(
            f'ray worker: {global_rank=} {rank=} {world_size=} {torch.cuda.current_device()=}; {torch.cuda.get_device_name(device)=}; {torch.cuda.get_device_capability(device)=}'
        )
        ...
        if global_rank == 0:
            ...
            print('tensor model parallel size: ',
                  mpu.get_tensor_model_parallel_world_size(),
                  mpu.get_tensor_model_parallel_rank())
            print('pipeline model parallel size: ',
                  mpu.get_pipeline_model_parallel_world_size(),
                  mpu.get_pipeline_model_parallel_rank())
            print('data parallel size: ', mpu.get_data_parallel_world_size(),
                  mpu.get_data_parallel_rank())
            print('virtual pipeline parallel: ',
    ...
    def init_model(self, config):
        ...
        self.parallel_layer = ParallelLlamaMLP(config=config,
...
if __name__ == '__main__':
    ...
    print(resource_pool.max_collocate_count, resource_pool.use_gpu,
          resource_pool.world_size)
    pg_scheme = [[{
    ...
    x = torch.rand(size=(seq_len, batch_size, hidden_size),
                   dtype=torch.float32)
