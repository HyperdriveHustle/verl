    ...
    def collect_all_layers(self):
        ...
        if self.rank == 0:
            for layer_idx in range(self.world_size):
                ...
                temp_layer = SimpleLayer(layer_idx).to(f'cuda:{self.local_rank}')
...
def main():
    ...
    print(f"[SETUP] on rank {rank} (local_rank: {local_rank}), world_size: {world_size}")
    ...
    if rank == 0:
        ...
        if diff < 1e-5:
            print("Validation successful! Distributed and local results match.")
        else:
