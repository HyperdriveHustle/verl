...
def train_with_full_batch(model, X, y, epochs=1, batch_size=32):
    ...
    initial_grads = [param.grad.clone() if param.grad is not None else None 
...
def train_with_gradient_accumulation(model, X, y, epochs=1, batch_size=8, accumulation_steps=4):
...
def main():
    ...
    grad_accum_initial_grads, m2 = train_with_gradient_accumulation(model2, X, y)
