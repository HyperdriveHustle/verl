...
def benchmark_nccl_communication(begin_size, end_size, factor, local_rank, num_tests=10):
    ...
    tensor = torch.rand(size // (torch.finfo(torch.float32).bits // 8), device=f'cuda:{local_rank}')
    for _ in range(5):
    ...
    while size <= end_size:
        ...
        if local_rank == 0:
            display_log(f"NCCL Performance Test Round {i}: PASS - Bus Bandwidth: {busbw:.2f} GB/s")
            persistene_log(f"NCCL Performance Test Round {i}: PASS - Bus Bandwidth: {busbw:.2f} GB/s")
...
def main():
    ...
    parser.add_argument("-b", "--begin-size", default="8", help="Starting data size (e.g., 8, 128M, 1G).")
    ...
    try:
        ...
        print(local_rank)
        benchmark_nccl_communication(args.begin_size, args.end_size, args.factor, local_rank)
