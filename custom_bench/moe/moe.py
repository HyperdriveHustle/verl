...
class DistributedMoE(nn.Module):
    def __init__(self, input_dim, output_dim, num_experts, world_size):
        ...
        self.local_expert_indices = list(range(
            dist.get_rank() * experts_per_process,
        ...
        print(f"Process {dist.get_rank()} handles experts {self.local_expert_indices}")
    ...
    def forward(self, x):
        ...
        local_outputs = torch.zeros(batch_size, self.output_dim, device=x.device)
