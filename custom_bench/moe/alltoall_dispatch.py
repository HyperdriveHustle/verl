...
def dispatch_tokens_to_experts(tokens, expert_assignments, num_experts, world_size, rank):
    """
    ...
    for r in range(world_size):
        ...
        if len(padded) < max_tokens:
            ...
            padding = torch.zeros((max_tokens - len(padded), hidden_dim), device=tokens.device)
            padded_tensor = torch.cat([torch.stack(padded) if padded else torch.tensor([], device=tokens.device), 
                                      padding], dim=0)
        else:
...
def main():
    ...
    processed_tokens = dispatch_tokens_to_experts(tokens, expert_assignments, num_experts, world_size, rank)
